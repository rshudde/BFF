...
plot_BFF = function(effect_size, BFF,  save = FALSE, savename=NULL, xlab = NULL, ylab=NULL, main=NULL, r = NULL){
  ...
  #Adding rectangles
  p <- p + annotate("rect", xmin = -1, xmax = 0.1, ymin = -Inf, ymax = Inf, fill = adjustcolor("red", 0.1))
  p <- p + annotate("rect", xmin = 0.1, xmax = 0.35, ymin = -Inf, ymax = Inf, fill = adjustcolor("orange", 0.1))
  p <- p + annotate("rect", xmin = 0.35, xmax = 0.65, ymin = -Inf, ymax = Inf, fill = adjustcolor("blue", 0.1))
  p <- p + annotate("rect", xmin = 0.65, xmax = 2, ymin = -Inf, ymax = Inf, fill = adjustcolor("green", 0.1))
  p <- p + geom_vline(xintercept = c(-1, 0.1, 0.35, 0.65, 2), lwd = 0.2)
  ...
  p <- p + scale_y_continuous(breaks = axis_position, labels = axis_labels)
