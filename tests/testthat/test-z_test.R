test_that("two-sample: basic functionality", {
  ...
  testthat::expect_equal(fit$log_bf, -0.27839, tolerance = 1e-5)
  ...
  # check that the BF and omega is consistent
  testthat::expect_equal(fit$log_bf, 0.29278, tolerance = 1e-4)
  testthat::expect_equal(fit$omega,  0.50)
  ...
  # check that the BF and omega is consistent
  testthat::expect_equal(fit$log_bf, 2.07857, tolerance = 1e-5)
  testthat::expect_equal(fit$omega,  0.45)
  ...
  testthat::expect_equal(
    ...
    c(
      "\tBayesian non-local two-sample z test"  ,
      ""                                        ,
      "maximized log Bayes factor = 2.08"       ,
      "maximized omega = 0.45 (Cohen's d)"      ,
      "alternative = two.sided"
    )
  )
